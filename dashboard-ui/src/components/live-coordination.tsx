...
function concreteModel(value: string) {
...
function ActiveAgent({ node, onOpen }: { node: TraceNode; onOpen: (node: TraceNode) => void }) {
  return <button onClick={() => onOpen(node)} className="w-full rounded-[6px] border bg-background p-4 text-left transition-colors hover:bg-accent"><div className="flex min-w-0 flex-wrap items-start justify-between gap-2"><div className="min-w-0"><p className="break-words text-sm font-medium">{node.role}</p><p className="mt-2 break-words text-xs leading-5 text-muted-foreground">{node.task || "Waiting for task detail"}</p></div><StatusBadge value={node.status} /></div><dl className="mt-4 grid gap-3 border-t pt-3 text-xs sm:grid-cols-3"><div><dt className="text-muted-foreground">Model</dt><dd className="mt-1 break-all font-mono">{concreteModel(node.resolved_model || node.model || "Pending")}</dd></div><div><dt className="text-muted-foreground">Runtime</dt><dd className="mt-1 break-words">{node.harness} · {node.resolved_backend || node.provider}</dd></div><div><dt className="text-muted-foreground">Elapsed</dt><dd className="mt-1 font-mono">{formatMs(node.duration_ms, node.started_at, node.completed_at)}</dd></div></dl></button>
}
