...
def normalize_routing_policy(value: Any) -> dict[str, dict[str, str]]:
    ...
    for lane in LANE_ROLES:
        ...
        if target in {"ollama", "openrouter"} and not model:
            raise RoutingPolicyError(f"choose a concrete {target} model for {lane}")
        if target not in {"ollama", "openrouter"}:
            model = ""
        normalized[lane] = {"target": target, "model": model}
    return normalized


def load_routing_policy(ledger: Ledger) -> dict[str, dict[str, str]]:
    ...
    try:
        return normalize_routing_policy(raw)
...
def apply_routing_policy(
    ...
    *,
    ...
    for lane, roles in LANE_ROLES.items():
        ...
        if target == "auto":
            continue
        if target in {"ollama", "openrouter"}:
            runtime = "open_code"
